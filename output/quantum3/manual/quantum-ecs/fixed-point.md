...
Quantum's FP Math uses carefully tuned look up tables for fast trigonometric and square root functions (see `Assets/Photon/Quantum/Resources/LUT`).
...
This will internally parse the `string` as a float and then convert to `FP`. All caveats from `FromFloat\_UNSAFE()` apply here as well.
...
The snippets needed for converting a regular Unity `AnimationCurve` into an `FPAnimationCurve` can be found [HERE](/quantum/current/concepts-and-patterns/animation-curves-baking).
