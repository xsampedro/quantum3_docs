...
func New(config Config) *Scraper {
	...
	// Use default content selectors if not specified
	if config.ContentSelectors == "" {
		config.ContentSelectors = "main, article, .content, .main-content, #content, div[role='main'], body"
	}
...
func (s *Scraper) shouldProcessURL(url string) bool {
	...
	for _, pattern := range s.config.ExcludeURLPattern {
		if strings.Contains(url, pattern) {
			return false
		}
	}

	// Check if URL contains the base path
	...
	if !s.config.ListOnly {
		s.collector.OnResponse(func(r *colly.Response) {
			...
			doc.Find("body").Each(func(i int, sel *goquery.Selection) {
				...
				// Remove additional site-specific elements
				sel.Find(".product-menu, .col-md-3, .multiplayer-menu, .join-circle, .photon-footer").Remove()

				// Find main content using content selectors
				content := sel.Find(s.config.ContentSelectors)
				if content.Length() > 0 {
